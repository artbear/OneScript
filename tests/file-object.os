...
Процедура ТестДолжен_ПроверитьАтрибутПутьУФайлаВКорневомКаталоге() Экспорт
	...
	ИмяФайла = "C:\Program Files (x86)\1cv8\8.3.8.1675\bin\";
	//ИмяФайла = ОбъединитьПути(ТекущийКаталог(), "C:\Program Files (x86)\1cv8\8.3.8.1675\bin\");
	
	Пока Истина Цикл
		Файл = Новый Файл(ИмяФайла);
		//Сообщить("ИмяФайла " +ИмяФайла);
		Сообщить("ИмяФайла " +ИмяФайла +", Файл.Путь " +Файл.Путь);
		Если Файл.Путь = ИмяФайла Тогда // 1С возвращает для корневого каталога самого себя
			//юТест.ПроверитьРавенство(Файл.ПолноеИмя, "C:\", "Файл.ПолноеИмя");
			//юТест.ПроверитьРавенство(Файл.Путь, "C:\", "Файл.Путь");
			Прервать;
		ИначеЕсли ПустаяСтрока(Файл.Путь) Тогда
			ВызватьИсключение "Файл.Путь в 1С не должен возвращать пустую строку";
		КонецЕсли;
		ИмяФайла = Файл.Путь;
	КонецЦикла;
	юТест.ПроверитьРавенство(Файл.Путь, "C:\", "Файл.Путь должен возвращать корневой каталог диска, а это не так");
КонецПроцедуры
